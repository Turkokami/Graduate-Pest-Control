---
slug: huntington-station-rodent-control
title: Rodent Control in Huntington Station
metaTitleCore: Huntington Station Rodent Control
quickAnswer: >-
  Huntington Station runs two rodent problems on one street grid. Rats work the commercial corridor
  and the partly unsewered downtown from below grade; mice work eleven thousand mixed housing units
  from the sill line up. Neither is solved by the other's method, and in a hamlet where a fifth of
  homes are rented, the fix usually needs the owner.
metaDescription: >-
  Rodent control in Huntington Station, where rats work a partly unsewered downtown from below grade
  and mice work eleven thousand mixed housing units.
faqs:
  - question: Is it rats or mice in Huntington Station?
    answer: >-
      Both, on the same grid, in different places. Rats hold a year-round baseline on the New York
      Avenue and Route 110 frontages and in the downtown south of the tracks. Mice are the
      residential problem across the post-war belt and the older streets, arriving in autumn.
  - question: Why does the downtown behave differently from the rest of the hamlet?
    answer: >-
      Because a great deal of it sits on private drainage rather than a public sewer, and because
      half a century of demolition and repaving left services in the ground under surface parking. A
      rat below grade there is moving through pipework and voids that belong to no single building.
  - question: My upstairs tenant has mice and my downstairs flat does not. Why?
    answer: >-
      Because the entry and the evidence are rarely on the same floor. A house converted to two
      dwellings has a second waste stack and a second service run cut vertically through floors that
      were never closed, so an animal entering at the rear cellar wall feeds two stories above it.
  - question: Does a new apartment building have fewer rodent openings?
    answer: >-
      Fewer, and larger. A podium building is tight across most of its skin, which concentrates
      entry at the garage ramp, the loading and refuse doors, the sprinkler and utility penetrations
      and the elevator pit. Those are the points worth the money on a building of that type.
  - question: Do the commuter parking lots make it worse?
    answer: >-
      They contribute food and shelter rather than a population of their own. About 5,000 commuters
      head west from this platform in the weekday peak, and a parking deck supplies sheltered
      ground-level runs along its edges, planters, and refuse cleared on a collection round rather
      than continuously.
  - question: How quickly does a mouse population announce itself indoors?
    answer: >-
      Inside a fortnight, usually. A mouse leaves fifty to sixty droppings in a day on Cornell's
      figure, and the oily sebum marks that build along a frequently used run are visible not long
      after. The suddenness is a matter of visibility, not of breeding speed.
  - question: I rent. What can I actually do about this?
    answer: >-
      Record it and report it in writing. Most tenancies here come with no key to the cellar, no way
      into the risers, no roof access and no control of the yard, which is exactly where the defect
      sits. Authorising a repair falls to the owner or the managing agent.
  - question: Do I need a permit to have this work done?
    answer: >-
      Not for trapping, sealing or screening. You do once the repair turns into construction:
      renewing decayed sill framing, forming a vent, rebuilding a stoop, or holding grade with a
      wall. There is no village government here, so any filing goes to the Town of Huntington.
city: huntington-station
service: rodent-control
sources: []
gatePassed: false
---

## Why does this hamlet run two rodent problems instead of one?

Because the buildings are not one kind of building, and the two animals answer to different parts of the fabric.

Almost everywhere else in the Town of Huntington, a rodent call resolves into a single question about a detached house. Here it does not. The 2020 census counted 34,878 people across 5.48 square miles, about 6,369 to the square mile, and the housing under them is genuinely mixed: some 11,414 units at a median build year of 1961, of which only 72.5 per cent are detached. There are 1,316 attached units. About 1,031 more sit inside buildings of two to four dwellings and 766 inside buildings of five or more, while some 2,394 — 22.1 per cent of the stock — are let.

That mixture splits the work in two.

**The mouse job** runs across the residential streets and is a seasonal, envelope-shaped problem. It starts at the point where timber meets masonry, it peaks between October and December, and on the post-war belt it repeats house to house predictably enough to scope from the curb.

**The rat job** runs along the corridors and through the downtown, and it is not seasonal in any useful sense. It is a below-grade and infrastructural problem with a year-round baseline that rises in autumn rather than beginning then.

Confusing the two costs money in a specific way. A rat problem treated as a mouse problem gets an envelope scope on a building whose real defect is a broken line under the floor. A mouse problem treated as a rat problem gets exterior stations along a fence line while the animals are living in a chase behind a kitchen. The identification question is therefore not academic here, and it is the first thing we settle. The species differences that drive it are set out under [rodent control](/pest-control/rodent-control/), with the field detail in the [Norway rat](/pest-library/norway-rat/) and [house mouse](/pest-library/house-mouse/) profiles.

## What did the clearance around the station leave in the ground?

Old services, old foundations and a great deal of sheltered edge — all of it under asphalt.

Sixty years ago the urban renewal program took 86 businesses off the ground around the railroad station. Mixed-use redevelopment was supposed to replace them; commuter parking did instead, and the Town's own revitalisation application is blunt about the fifty years of disinvestment that came after. New York Avenue was widened in a way that cut the corridor's two sides apart from each other.

Read that as a construction history rather than a civic one and it produces a short list of findings that turn up again and again on the blocks nearest the platform.

**Abandoned laterals.** A building demolished at grade leaves its drain connection in the ground. Some were capped properly. Some were capped with whatever was in the truck. Sixty years later, a capped and forgotten line running to a live main is a below-grade route with nothing at the end of it that anybody inspects.

**Voided fill over old cellars.** Where a cellar was backfilled quickly, the fill settles, and a settled void under a slab or an asphalt surface is burrow space that never shows on the surface until the surface cracks.

**Long sheltered edges.** Surface parking and two multi-story garages — one raised on the south side in the 1980s, a second on the north side during 1990s work — give continuous curb lines, planters, deck edges and refuse points. Rats travel with a flank against something vertical, and a parking lot is almost entirely vertical edges.

None of that is a reason to despair of the downtown. It is a reason to survey it below grade rather than around it. On the blocks by the station, the productive half hour is spent on drainage, cellar floors, hatches and the ground under the building, not on the walls of it.

## What is a rat traveling through south of the station?

Private drainage, mostly — which changes both the diagnosis and who fixes it.

The southern portion of the downtown is not sewered. The county has approved $1.25 million to carry a comprehensive sewer scheme for the ground south of the station through its engineering and design stage, and puts the construction figure somewhere between $30 and $40 million. Until that is built, the densest commercial fabric in the town discharges to on-site systems and private laterals.

That is a different picture from the city model, where a building's connection to a public sewer is the door and a cracked house trap is the finding. The equivalent structures here are on the parcel itself: a tank or a ring, the risers and covers over it, a distribution box on many properties, a leaching field or gallery, and the run connecting the building to all of it. Sound and intact, none of that is a pest problem. What creates one is a settled riser holding water under a cover, a cracked or displaced lid, a lateral broken by root or vehicle load, or a system that was undersized when the ground floor was a shop and is now serving a food premises.

Two practical points follow.

The first is that the survey has to go outside the building and into the yard, and the yard behind a commercial frontage on New York Avenue is where the refuse enclosure, the grease container, the service door and the wastewater covers all sit within a few yards of each other. That is a complete rat habitat in about two hundred square feet, and it belongs to a tenant who did not build it.

The second is that a proportion of these findings are not ours to repair, and saying so early is worth more than treating around them. A broken lateral is a drainage contractor's job. A failing on-site system is the business of the people who service it. Our part is to find it, record it, close the building against what it is producing, and name who should be called. Food businesses along the corridor sit inside [restaurant pest control](/commercial/restaurant-pest-control/), within the wider [commercial program](/commercial/).

## Why does the evidence turn up in the wrong apartment?

Because a converted house moves animals vertically, and the conversion cut the routes.

Around a thousand units here sit in buildings of two to four dwellings, and the great majority of those buildings were framed as one house. Turning a single-family frame into two dwellings means a kitchen where none was framed for, a bathroom set above a bedroom, and an extra waste stack, gas line and electrical feed carried up through floor structures that were never closed at any level.

What that produces is a chase. Not a designed shaft — a series of holes cut through joists and top plates by different trades on different days, connected by the voids that were already there. A mouse entering at the rear cellar wall has a straight run up that chase to the top floor without ever crossing an occupied room.

The pattern this generates is consistent and routinely misdiagnosed. The upstairs tenant reports droppings in a kitchen cabinet. The upstairs unit is treated. The animals in that unit are reduced. Within a few weeks the ground-floor unit reports the same thing, and then the upstairs unit again. Nobody has done anything wrong; the scope simply addressed a compartment inside a building that behaves as one volume.

The fix is not more visits. It is closing the chase at every floor level it crosses, closing the joist pockets where the floor structure enters a party or dividing wall, fitting proper escutcheons where supply pipes come through finishes, and closing the entry at the cellar wall that started it. That is a building-scale scope and it is set out for this hamlet at [structural exclusion in Huntington Station](/pest-control/huntington-station/structural-exclusion/).

There is a second consequence worth naming. In a converted house, the cellar is often the landlord's and the top floor is often the tenant's, so the person seeing the evidence and the person holding the key to the room where the entry is are two different people who may not have spoken in a year.

## Why does the newest building on the block report first?

Because a tight envelope concentrates the traffic, and concentrated traffic is visible.

For a hamlet this size the recent building program is substantial. Avalon Bay finished 303 rental apartments in 2013, 44 of them affordable. Country Pointe added 76 equity townhomes. Northridge holds 16 one-bedroom apartments above 6,500 square feet of retail, Gateway Plaza 66 apartments above 16,000 square feet of commercial floor, and Columbia Terrace 14 affordable condominiums for military veterans. A 2017 town resolution requires a fifth of the apartments in commercial-district mixed-use buildings to be affordable.

Those buildings are better built than what surrounds them, and that is exactly why their rodent problems look dramatic when they happen. On an older house the pressure spreads across dozens of small defects and nothing looks like a highway. On a podium building there are five or six places where the skin is deliberately open, and everything that gets in uses one of them.

**The garage ramp and its door.** A vehicle door that closes on a sensor, on a ramp that runs below grade, at a threshold that collects grit. It is open several times an hour at peak.

**Refuse and loading.** A refuse room with a door held open during collection, a compaction or bulk store, and a service corridor connecting it to the core.

**The utility and sprinkler penetrations.** Cored openings through a below-grade wall, sleeved to code, sealed to nobody's satisfaction.

**The elevator pit and the lowest core level**, which is the wettest and least-visited part of a new building.

**The commercial tenancy at street level**, which arrived after handover with its own fit-out, its own penetrations and its own refuse arrangement.

There is also a handover problem specific to new buildings. By the time the first population turns up, the people who built the thing have moved on, the fit-out trades who made the final cuts were answering to a tenant rather than the base build, and whoever now carries the problem holds no record of either. On a building under fifteen years old, half the survey is establishing what was cut and by whom.

The good news is that the list is short and the items are large, so a building of this type can genuinely be closed. The bad news is that continuous corridors and risers mean one apartment's problem is the whole building's problem in a fortnight, which makes a complaint-driven schedule the wrong instrument. That is a managing agent's decision, and the shape of it is described under [property management pest control](/commercial/property-management-pest-control/).

## What do the older houses need that a 1955 cape does not?

Masonry work, and an assumption that a route runs from the cellar to the attic without a break.

About 1,318 units here — 11.6 per cent of the stock — were built before 1940, and a further 1,173 in the 1940s. That is a larger share of genuinely old fabric than anywhere else in the town's inland belt, and it sits interleaved with post-war housing rather than in a district of its own.

On those buildings the foundation is frequently rubble, fieldstone or early block. The mortar was never a continuous seal and has been opening for eighty years or more. The sill sits on masonry with nothing between the two, so it wicks. And the framing above is often balloon-framed rather than platform-framed, which means a stud bay runs from the cellar to the roof without a fire stop or a plate to interrupt it. An animal that gets in at grade on that construction is not in a wall; it is in a vertical shaft that happens to be a wall.

The work is correspondingly different. It starts with the mortar rather than with a sealant gun: raking out and repointing failed joints, packing around the top of the wall where the sill bears, and closing the stud bays at the top of the foundation so the shaft is no longer continuous. It is slower than the post-war belt and it is the part of the hamlet where the closing work most obviously pays, because a route that runs three floors is worth more to an animal than a route that runs one.

There is a further wrinkle particular to this hamlet. Because the older houses sit next to post-war infill rather than in a preserved district, many of them have been re-clad. Aluminum or vinyl siding laid over the original weatherboard leaves a continuous vertical cavity behind it running from grade to the soffit, and on a balloon-framed house that cavity connects to the stud bays at the top. From the sidewalk the wall looks new. Nothing about the route inside it has changed, and no ground-level inspection will show that it exists.

By contrast the 3,039 units from the 1950s and 2,365 from the 1960s repeat the familiar post-war list — an ungasketed band joist, field-cut service penetrations, the garage-to-house junction, a shallow soffit with corners that have lost their fixings — and on those streets the survey confirms rather than searches.

## Who can actually authorise the fix?

In more than one case in five, not the person making the call — and that decides how the job is run.

With 2,394 units rented and around a thousand more in small conversions, a large share of the reports that reach us come from someone who cannot open the cellar, cannot reach the risers, cannot get onto the roof and does not control the yard or the refuse. A tenant can store food properly and keep a kitchen clean and still be entirely unable to change the outcome, because the opening is at the rear of a cellar they have never seen.

That is not a reason to treat the tenant's report as unreliable. It is usually the best evidence available, and it should be recorded rather than tidied away: where the droppings are, what date they appeared, whether the noise is above the ceiling or inside the wall, and whether it is daytime or after dark. What it does mean is that the conversation that changes anything is with the party who holds the building.

For an owner of a two-to-four-unit building, that conversation is short. The scope is the entire structure; the access needed is every unit plus the cellar inside one program; and the order is not negotiable, because closing a building that still has animals in it takes away their exit rather than the animals.

For a managing agent on a larger building, the equivalent is a written scope covering the envelope, the shared services and the refuse handling, with a record of what was done where. Owners putting this out to two or three firms should start with [how to verify a New York contractor and read a scope](/compliance/choosing-a-pest-control-contractor/), because on a building like this the gap between quotations is almost always an omission.

Consultation costs the caller nothing and is generally a phone call. A fee attaches only to the written proposal and plan. That document is what is being purchased,
      and its cost is deducted from the project if the work goes ahead.

## When does the work have to happen here?

Before the nights turn, on the residential side — and continuously on the corridor, because the corridor has no off season.

**Late August to November** is what sets the residential year. A house whose band joist, penetrations and garage junction are closed before the autumn move never acquires a winter population, which is a materially better position than opening a finished ceiling in February to reach one.

**December to February** is the sharpest diagnostic stretch on the older stock, because on a cold still morning a building advertises its openings by leaking heat through them, and those are the same openings the animals use. Under a pre-war house with an unlined cellar, the head of the foundation wall read by hand on a January morning gives up more than any summer inspection of the same building.

**March to May** is when the ground can be read: burrow entrances along a fence line, the winter's freeze-thaw record on a stoop and an apron, and — on the unsewered blocks — the covers and the run between building and field, all visible before planting fills in.

**June to August** is the exterior inventory. Refuse enclosures, the yard behind a shop, stored material against a rear wall, and the ivy and shrubs that have grown tight to a foundation on the older streets.

Two local schedule items sit across that calendar. Site disturbance moves rats, and this is a hamlet with an active building program and a sewer scheme in design; excavation next door is the moment to look at your own cellar wall rather than a month later. And if a burrow system is suspected beneath decking, a hardscape apron or a loading bay, [canine rodent detection](/pest-control/canine-rodent-detection/) will find it before anything is opened up.

## What does a finished job look like in this hamlet?

A closed list with an end date on the residential side, and a maintained baseline plus a named repair on the corridor. They are different deliverables and they should be sold as different things.

On a house — pre-war or post-war, owner-occupied or let — the work is finite. Reduce the population where the result can be retrieved, close the openings with material matched to the substrate, correct the ground conditions that made the property hospitable, and record the lot. The scope ends. If a service arrangement on a house in this hamlet looks the same in year three as it did in year one, nothing structural was ever done.

On a mixed-use or multi-unit building the honest version is longer. The envelope work can be finished. The refuse handling and the drainage need somebody to keep doing them, and where the actual source is a broken line, a neighbor's yard or a shared service area, the closing work protects the building without reducing what is outside it. Saying that plainly at the start is better than discovering it in month nine.

In both cases the record is the product. Openings located, each with the finding, the material and a photograph. Anything out of reach named, with the reason. Drainage and refuse findings set down against the trade that owns them. And a date to come back and look.

Sealing is dealt with at [structural exclusion in Huntington Station](/pest-control/huntington-station/structural-exclusion/), and the grade, drainage and clearance items above also settle the insect side of the older quarter, because the 1,318 units here that predate 1940 are where a carpenter ant colony finds framing soft enough to excavate — that work is [ant control](/pest-control/ant-control/), and in a hamlet with tenanted units it is often the landlord's repair rather than a treatment. Every service we run in the hamlet is indexed at [pest control in Huntington Station](/locations/huntington-station/); the town sits above it at [Huntington](/locations/huntington/), the region at [Long Island](/locations/long-island/), and the whole territory at [service areas](/locations/). South of here, [Dix Hills](/locations/dix-hills/) puts the opposite settlement pattern on the same county, and [Melville](/locations/melville/) shows what an office corridor does to the same soil.

If you have heard something in a wall, found droppings on a cellar sill, or been told the problem is next door's, [tell us the address, the floor and the date](/contact/).
