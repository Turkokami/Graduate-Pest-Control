...
faqs:
  ...
  - question: We run a shop at street level. Does our waste really matter that much?
    answer: >-
      It is usually the largest single variable. A retail and restaurant district generates organic
      waste in volume, and the Health Department is direct that exposed garbage attracts rats. Rigid
      containers with tight-fitting lids in sufficient number, stored so that they are not sitting on
      a vault covering, does more than any amount of material applied nearby.
  - question: Can we seal the vault lights to keep rats out?
    answer: >-
      Not as a general measure, and it would usually be the wrong repair. Panels that still deliver
      light have a function, exterior repairs in the historic district are reviewed by the Landmarks
      Preservation Commission, and the productive boundary is inside — the wall between the vault and
      the occupied cellar, with its service penetrations and stair head.
  - question: Rats appear inside with no opening we can find at ground level. How?
    answer: >-
      From below. A deteriorated house trap, a cracked or disconnected drain line under the cellar
      floor, a missing cleanout cover or a floor drain whose seal has dried out makes the inside of the
      building continuous with the sewer under the street. That route is invisible from outside and
      completely unaffected by exterior baiting.
  - question: Do we need the ground-floor tenant's cooperation, or can we work around them?
    answer: >-
      You need it. A store-and-loft building has one set of services from cellar to roof, so the
  ...
  - question: When should this work be scheduled?
    answer: >-
      Late summer into autumn for anything below grade, because populations move toward buildings as
      the weather cools and a boundary closed before that never acquires a winter population. Spring
      is for the vault covering and the roof after a season of freeze and thaw, and summer for waste
      storage and drainage repair.
city: soho
...
## Why does the sidewalk decide most of this work?
...
Volume, frequency and a second decision-maker in every building.

This is one of the busiest shopping and eating districts in the city, and it is arranged vertically. The store-and-loft type puts a shop at street level, working floors above and a single set of services from cellar to roof. Conversion put apartments upstairs and left the commercial tenancy where it always was. On most blocks that tenancy is food service or high-turnover retail with a back-of-house area.

Three consequences follow, and only one of them is obvious.

**The food supply is industrial in scale rather than domestic.** A residential building of a dozen households produces a predictable quantity of waste on a predictable schedule. A restaurant produces organic waste continuously and puts it out at a time set by collection rather than by convenience. Where that waste sits on a vault covering, the shelter and the food are in the same place.
...
**The vault-to-cellar boundary itself.** Where it is a masonry wall, it needs to be intact — no bricked opening that was closed with something soft, no gap where a beam or a duct crosses.

**The stair or hatch out of the vault.** Where a route exists from the vault up into the occupied cellar, nothing else in the building matters as much.

**Abandoned delivery chutes and pipe sleeves.** These buildings received goods below grade and have been re-serviced repeatedly. Every abandoned line left a hole.

**The house drain, its trap and its cleanouts.** A deteriorated house trap, a broken line under the floor slab, a missing cleanout cover or a dried-out floor drain each makes the inside of the building continuous with the sewer.

**Standing water.** A cellar or vault holding water will re-attract whatever the sealing excluded, and it is also why the below-grade cockroach species are permanent residents here rather than visitors — see [cockroach control in SoHo](/pest-control/soho/cockroach-control/).

**Service door thresholds.** Sweeps closing gaps to no more than a quarter of an inch, which is both sensible and the standard [Local Law 55](/compliance/nyc-local-law-55/) sets.

Materials matter as much as locations. Cured expanding foam is not a barrier — it is chewed through easily, and it causes a second failure by looking like completed work, so a live opening gets recorded as closed. Barriers are things an animal cannot bite: mesh, hardware cloth, sheet metal, mortar, hydraulic cement. Foam is not on that list and is not used here in a supporting position either. That principle is the whole of [structural exclusion](/pest-control/structural-exclusion/) and it applies with particular force in a building where nobody will look at the work again for a decade.

## Are mice a separate problem from the rats?

Entirely separate, with a different route, a different scope and usually a different budget.

Rats are the below-grade, boundary and drainage question described above. [House mice](/pest-library/house-mouse/) are an internal, vertical question, and in this stock they live in the features that made the buildings work as factories.

**The floorplate was one room.** Loft floors were single undivided spaces. The partitions that later made apartments out of them generally stop at a dropped ceiling rather than at the structure above, so above the line of the walls the floor is still continuous, and so is the cavity above the ceiling with everything that has been run through it since.

**The shafts run the full height.** Goods and later passengers moved vertically through shafts cut through masonry buildings — this district has an unusually early history of that, and the arrangement has never changed. A decommissioned shaft that was boarded at one or two levels survives as a continuous cavity from cellar to roof. A shaft that was repurposed as a service riser has an opening at every floor, which is worse.
...
By working from the inside out, and by treating the protected fabric as a constraint on where you fix things rather than on whether you fix them.

The cast-iron front is, in most of these buildings, a face bolted onto the structure rather than the structure itself. After a period of fires, an 1899 building code required masonry backing behind cast-iron fronts, so on many buildings there is iron, then a bedding or cavity layer, then brick. That is a plane running the full height of the facade where water travels, where fixings pass through, and where a repair made in one decade does not necessarily meet a repair made in another.

For rodent work that matters in a limited and specific way: it is a water path into the wall behind, and wet structure inside a masonry envelope is a different problem from a rodent one. What it changes procedurally is that any sealing specified on a landmarked elevation is being applied across two assemblies rather than one, and has to be reversible and set back rather than smeared across the face.

Three rules cover almost every situation in this district.

**Do the concealed work first, because it needs no review and carries the traffic.** Vault interiors, cellar walls, shaft bases, floor collars, roof bulkheads and rear elevations are invisible from the public way.
...
## How do you read the evidence at street level here?
...
Reduce, close, correct the water, verify — and the sequence is not optional.

**Reduce before you close.** Sealing a building that already holds an interior population removes the animals' way out, and they concentrate in wall voids and shaft cavities where anything that dies is difficult to reach. Trapping and monitoring come first, run long enough to establish both the size of the population and its routes.

**Read the evidence, not the sightings.** Rub marks along a wall base, droppings graded by size and age, gnawing at a particular corner and the polished look of a route in use are the map. A cellar swept the morning of the survey has erased it. Moving stored goods a foot back from the perimeter is useful; cleaning the floor is not.

**Close in the right order.** The vault boundary and the drainage first, because they are the highest-volume routes; then the shaft base and head and the floor collars; then the thresholds.

**Correct the water while the wall is open.** Drainage repair is inside the rodent scope here rather than alongside it.

**Verify with monitors.** Absence of complaints is not evidence. Monitors returned to the same locations and read again tell you whether a population collapsed or moved along the blockfront to the next building.

And then the part that no single owner can do alone. Because the vault is continuous and the buildings are not, a coordinated blockfront — several owners closing their boundaries and rebuilding their waste storage in the same season — achieves something that no individual building can. That is a slow conversation and it is worth starting, because the alternative is each owner in turn paying to move the problem four doors along.

## What does a board or an owner here actually have to do?

More than react, and the duties attach to the building rather than to the residents.

Once a converted loft building is a multiple dwelling, the Housing Maintenance Code places the duty to keep the premises free of rodents, insects and other pests on the owner — in a cooperative, the corporation. The substance sits at [NYC Housing Maintenance Code — extermination and rodent eradication](/compliance/nyc-housing-maintenance-code-pests/).
